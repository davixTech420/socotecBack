...
exports.createGroup = async (req, res) => {
  try {
    ...
    if ((await Group.findAll({ where: { nombre } })).length > 0) {
...
exports.deleteGroup = async (req, res) => {
  try {
    ...
    if ((await Group.findAll({ where: { id, estado: true } })).length > 0) {
      return res.status(400).json({ message: "No se puede eliminar el grupo porque esta activo" });
    }
    if ((await Proyect.findAll({ where: { groupId: id } })).length > 0) {
...
exports.inactivateGroup = async (req, res) => {
  try {
    const { id } = req.params;
    if ((await Proyect.findAll({ where: { groupId: id,estado:true } })).length > 0) {
      return res.status(400).json({ message: "No se puede inactivar el grupo porque tiene proyectos asociados" });
    }
